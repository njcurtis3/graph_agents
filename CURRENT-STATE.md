...
> was deliberately left for later because it reads `activity.jsonl` and gap #20 was open.
> Built now, scoped around the gap rather than waiting on it — see the Decisions log and gap
> #20's own entry. Re-measured `wc -l` for the roster row (6 skills, was 5) and the new tool
> row. Ran `audit-fleet.py` afterwards: **117 claims checked, 2 drifted**, both pre-existing
> and already booked at the Bash write classifier row (`bash_write_targets.py`/
> `measure_bash_corpus.py` read as unregistered hooks) — nothing this pass touched is new
> drift.
>
> **What the second 2026-09-09 pass covers** (direct edit, no graph run — a per-node read
> contract plus one model-tier change is below the stop-rule threshold). `brief.py` gained
> `--for <node>[:<slice>]`: a per-node brief derived from `state.json` (goal, that node's
> plan entry, scout facts or the builder's report as the node needs them, prior review
> attempts) instead of the whole file, cutting what a node reads on start from ~168KB to a
> few KB on `2026-09-06-bash-write-guard`, the run it was measured against. `GRAPH.md` §3's
> contract and all six agent files' read steps now point at it, `state.json` only for what
> the brief omits. `builder` moved opus → sonnet: the argument is that a builder executes a
> human-approved slice rather than deciding anything, and the never-downgraded `reviewer`
> exists to catch what it misses — `GRAPH.md` § Model tiering carries the argument in full,
> not restated here. Re-measured with `wc -l`: `GRAPH.md` 366→386, `brief.py` 479→680,
> `architect.md` 96→97, `builder.md` 89→91, `reviewer.md` 97→100, `integrator.md` 52→55,
> `ops.md` 54→56. `--for` was exercised by hand against `2026-09-06-bash-write-guard` for
> all six node types and its two error paths (unknown node, missing slice on
> builder/reviewer); **not** added to `test_brief.py` — booked, not closed. `audit-fleet.py`
> reports clean on everything this pass touched. Fixed two pieces of drift this pass did not
> cause but found while re-running the auditor: the Status headline said *twelve runs*
> against 13 directories, and the Runs table did not list `2026-09-06-bash-write-guard` —
> the same two-drift shape three consecutive closes have now produced; added that run's row
> from its own `state.json` `log`.
>
> **What the second 2026-09-09 pass does NOT cover.** No narrative section of any run was
> re-read; the gap list, the Decisions log and every per-run *what happened* section stand
> as the first 2026-09-09 pass left them. `--for`'s per-node shape was designed against this
> file's schema and one real run, not measured across a diamond or a re-reviewed re-run —
> the builder brief's "prior review attempts" section is untested against an actual REJECT
> loop through the new code path. The two pre-existing auditor complaints about
> `bash_write_targets.py`/`measure_bash_corpus.py` being unregistered hooks are unchanged
> from the first pass and still booked there, not here.
>
> **What the first 2026-09-09 pass covers.** Run `2026-09-06-bash-write-guard`, three sequential
...
>
> **What the second pass does NOT cover.** No run was executed and no run state changed — this was a direct edit, below the stop-rule threshold, on hook + definition files. The gap list's reasoning, the per-run *what happened* sections and the Decisions log were **not** re-read. The 12 historical commits carrying trailers are **recorded, not cleaned** (gap #21). And the guard's own blind spots are real and stated rather than closed: it cannot see a PR body, a release note, a tag message, or a message typed into an editor — `ops.md` now carries that half in prose, which is exactly the arrangement this whole change was made because it does not hold.
>
> **What the 2026-09-06 pass covers.** Run `2026-09-06-fleetview-followups` closed, and this pass is the drift it caused: the Status headline still said *eleven runs* against 12 directories on disk, and the Runs table did not list the run at all — the same two-drift shape `2026-09-04-fleetview-payload` produced, which is now the third consecutive close to produce it. Both fixed; `audit-fleet.py` re-runs at **103 claims checked, none drifted**. Verified in `git` that `s1-record-activity-deferral` and `s2-scope-exceptions-member-guard` are both ancestors of `fleetview` master at `ed38d7e`, and re-ran `node fleetview/test.js` on merged master at **212 assertions, 0 failures** (193 before the run). Independently re-derived `s2`'s central safety claim rather than taking the reviewer's word: extracted `PROSE_OPENER`/`isRealPath`/`realScopeExceptions`/`scopeExceptionsMalformed` out of `index.html` and ran them against every real run state beside a literal copy of the old predicate — **0 of 12 runs change their banner decision**, and both docstring-only runs correctly stay silent. Also confirmed `serve.py` is byte-identical to the run's baseline `7eb4b20`, which is the fact `s1`'s merged paragraph depends on. **Gap #13 was independently rediscovered** by `s2`'s reviewer (the plan-scope guard is registered `Write|Edit`, so a Bash heredoc writes unwatched); it was already on this list and is left as it stands, not duplicated.
>
> **What the 2026-09-06 pass does NOT cover.** No line counts were re-measured this pass — no fleet definition file changed, and the checker's `(N ln)` family passed untouched. The narrative claims, the gap list's reasoning, the per-run *what happened* sections and the Decisions log were **not** re-read. One fact this file does not record and a reader should not infer: `fleetview` master is **ahead of `origin/master`** by both merges plus a pre-run crash fix, so the work is closed and merged but **not pushed**.
>
> **What the 2026-09-05 pass covers.** Re-measured with `wc -l`: `verify-state.py` 420→560, `brief.py` 392→479, `close-run.py` 240→254, `test_close_run.py` 204→289, `_schema.json` 31→32, `reviewer.md` 85→97, `integrator.md` 45→47, plus two new suites (`test_verify_state.py` 238, `test_brief.py` 191). Re-ran all five fleet suites on merged master, all exit 0. Verified in `git` that each of the five `feat/attempt-*` branches is an ancestor of `master` at `ea53381`, and the three `feat/split-*` branches of `fleetview` master at `b261a7f`. Re-derived `fleet-hardening`'s blocker count against the new resolver (7→3) and confirmed by hand that both survivors are the real findings. Added the two runs closed today to the Runs table and corrected three rows that claimed `close-run` and `brief.py` had never been used for real — they had, that same day.
>
> **What it does NOT cover, stated because the checker cannot say it.** Every narrative claim in the gap list, the per-run *what happened* sections and the Decisions log was **not** re-read this pass. The roster's execution counts were bumped to the auditor's measured values (scout 11, architect 11, builder 32, reviewer 31, integrator 1, ops 0), which are counts of *written keys*, not a judgment that each node did its job. `close-run/SKILL.md:47` is **knowingly stale**: its blocker table still says a `REJECT` means the re-review never landed, which `2026-09-05-review-attempts` s2 made impossible. The human dropped that file from s5's scope at the gate; it is recorded as debt in that run's log and is not fixed here.
>
...
> history.
> A **fourth 2026-09-03 pass** added `/audit-fleet`, and it is the first pass where the
> mechanical half of this re-verification was done by a script instead of by hand.
> Verified directly: `audit-fleet.py` run against this file, reporting **94 claims checked,
> 8 drifted** — and all eight were confirmed by hand before anything was rewritten. Three
> line counts had been stale since 2026-09-02 (`guard-builder-scope.py` 279→326, its test
> 194→247, `new-app/SKILL.md` 72→96); the roster's `integrator` row still said **no** while
> this file's own prose and gap #2 had both said since 2026-08-26 that it ran; and **three
> closed runs were absent from the Runs table entirely**, one of them
> (`2026-08-31-mobile-first-criteria`) not mentioned anywhere in this file at all.
> `test_audit_fleet.py` was driven through 63 checks over 30 synthetic umbrellas, each a
> real `git init` fleet with a controlled committer date, including a case asserting the
> tool leaves every fleet file byte-identical. **Not exercised**: nothing fires it
> automatically — it is run by hand, like every other checker here, and the staleness hook
> still only nags.
> A **fifth 2026-09-03 pass** recorded the board being observed. Verified directly: the
> board's appearance reported from a fresh session; the dispatch-vs-return timing measured
> across every `Agent` event in all five runs carrying a heartbeat (32 of 32 within 0.2s of
> a `SubagentStart`) rather than inferred from one; the four line counts of the files
> corrected in that pass re-measured; and `/audit-fleet` re-run to green afterwards, which
> is the first time this file's own checker has policed a pass that edited it.
> **Not** re-checked on any 2026-09-03 pass: the registry's contents, the invariant checker
> and FleetView — untouched, and recorded below as their own passes found them. The guard's
> line counts were corrected this pass; its behaviour was not re-tested.
>
...
- `2026-08-31-mobile-first-criteria` against the fleet itself — **executed**, 3 slices, all
  PASS on attempt 1, merged `dd7ac0b`. Produced `conventions/mobile-first.md` and the
  registry's `ui` routing field.
- `2026-09-01-huntstack-mobile` against huntstack — **executed**, 4 slices, all PASS,
  branch `feat/mobile-expo-scaffold`. Closed its outstanding human-read gate on 2026-09-03
  against a physical Android phone, recorded in the run's own `log` rather than by editing
  a reviewer's key.
- `2026-09-02-date-accuracy` against huntstack — **executed**, 3 planned slices plus one
  off-plan (`s4`, CI), all PASS, merged `50a5299`. Closed with `s2`'s human-read gate
  explicitly **not** run, by the human's own decision, and said so at the close.

These three are the drift `/audit-fleet` was built to find, and it found them on its first
run: all three were `status: done` on disk while the Runs table listed five runs and this
paragraph said "five runs so far". `2026-08-31-mobile-first-criteria` was not mentioned
anywhere in this file. That is the 2026-08-26 failure repeating, in a file whose header
...
| Graph spec | live | `graph_agents/GRAPH.md` (386 ln) |
...
| Run board | live, **added 2026-09-03**, **used by real runs from 2026-09-04**, and since 2026-09-05 it draws a slice that looped. Renders one run's `state.json` + `activity.jsonl` as a ~10-line board: gate, goal, a line per node, a row per slice pairing build with verdict, and the in-flight lane. Authored by nothing — every value is derived, and the placeholder rules are imported from `verify-state.py` rather than restated, so "written" means the same to the board as to the audit. Verified against all 8 runs on disk (diamond, off-plan slices, an untouched template run, a `parked` run, runs with no heartbeat) plus a synthetic mid-flight fixture; ASCII fallback when the console encoding cannot take the glyphs. **Gained `--for <node>[:<slice>]` 2026-09-09** — a per-node brief (goal + that node's plan entry + the scout facts or builder report it needs + prior review attempts) instead of the whole `state.json`, cutting what a node reads on start from ~168KB to a few KB on the runs measured. Every node's read step in `GRAPH.md` §3 and its own `.md` now points here first, `state.json` only for what the brief omits. Manually exercised against `2026-09-06-bash-write-guard` for all six node types plus its two error paths (unknown node, missing slice); **`test_brief.py` was not extended to cover it** — booked, not closed | `graph_agents/.graph/brief.py` (682 ln), `graph_agents/.graph/test_brief.py` (191 ln), `feature-graph` § the board |
...
| Node heartbeat | live — `SubagentStart`/`SubagentStop` + a `*`-matcher `PostToolUse` entry append one line per node event to `.graph/runs/<run>/activity.jsonl`. Verified: events recorded with agent/tool, main-session writes recorded as `orchestrator`, silent on a closed run and with no open run. **Gained `parent` (`parent_tool_use_id`) 2026-09-09**, diagnostic for gap #20: 1,411 of 1,496 recorded `stop` events have no matching `start`, are stamped `orchestrator` for lack of `agent_type`, and interleave with live work — not what "a subagent finished" should look like. `parent` is logged, not yet read; the hypothesis it tests (a phantom nests under a real node's own `tool_use_id` rather than being orphaned) is unverified until a run produces one. **Gained `tokens` 2026-09-11** — a real per-agent-id running total, summed on every `tool`/`stop` event from that specific subagent's own transcript (`~/.claude/projects/<encoded cwd>/<session_id>/subagents/agent-<agent_id>.jsonl`, `message.usage` per turn), not from anything the hook payload itself carries — confirmed against the hooks reference that no usage field exists there at all. A real coupling to an undocumented, internal Claude Code storage layout, accepted deliberately (built for FleetView's per-node token counter) and unwritten on any read/parse failure rather than fabricating a count. **Gained `say` the same day** — the same transcript line's `message.content` text blocks, kept as the newest one seen, collapsed to one line and capped at 220 chars; a snapshot (later write replaces, never accumulates), unlike `tokens`' running total. Built for FleetView's live agent-caption feature; same silent-on-failure rule (`last_said_by` returns `None` rather than a fabricated quote). 23 checks total in `test_record_activity.py` (was 15): `encode_cwd` against the one observed real mapping, path construction, token summation and last-said-text extraction against synthetic transcripts (real usage/text lines + a torn line + a no-usage/no-text line + one over the 220-char cap), and `main()` end-to-end with `HOME` and `CURRENT` redirected into a temp fleet — an orchestrator-side event (no `agent_id`) and a missing transcript both correctly omit `tokens` and `say` rather than raising or writing a fake value. The transcript shape and location (`agent-<id>.jsonl` under a session's `subagents/`, `message.usage`/`message.content` per line) were confirmed against real files from past sessions on this machine before writing either reader; neither the token nor the caption path has yet fired against a live fleet run, since none was open while this was built. Visually verified in Chrome against a synthetic fleet directory (not a real run): the live caption renders, updates, freezes on the last value seen while live, and correctly shows nothing on a diamond's concurrent same-type builders | `.claude/settings.json`, `.claude/hooks/record-activity.py` (256 ln), `.claude/hooks/test_record_activity.py` (203 ln) |
...
|---|---|---|---|---|
| `scout` | haiku | Read, Glob, Grep, Bash, WebSearch, WebFetch | 67 | yes, 13 runs |
...
The `integrator` cell read **no** until 2026-09-03, when `/audit-fleet`'s first run
caught it contradicting both gap #2 and the What-is-live row three tables up, each of which
had said since 2026-08-26 that `integrator` ran in `archive-adapters`. One fact, three
copies, and only two of them were updated. The **counts** in that column are deliberately
left unchecked by the auditor — they are explicitly scoped to run 3 below, so a later run
outdates the number without falsifying the claim — but the yes/no is unscoped, load-bearing
and now machine-checked against written keys in every run's `state.json`.
...
retrieval. `reviewer` and `architect` are never-downgrade. **`builder` moved opus → sonnet
2026-09-09**, on the same argument as the scout downgrade but for a different reason: a
builder executes exactly one slice against a file set and a `done_when` a human already
approved at the gate, so the judgment call happened before it starts, and the
never-downgraded `reviewer` — always a fresh context — exists specifically to catch what
it misses. Not yet run against a live builder spawn as of this pass; the change is
frontmatter-only and the next `single-loop` or diamond run is what actually exercises it.

`builder` also grew 89→91 the same day, for `--for builder:<slice>` (below): step 1 now
names the brief instead of the raw `state.json` read. `architect` 96→97, `reviewer`
97→100, `integrator` 52→55 and `ops` 54→56 moved for the matching change in their own
protocol steps — each now reads its brief first and `state.json` only for what the brief
omits. `GRAPH.md` grew 366→386 for the same contract change written once in §3 rather
than six times.
...
**Since 2026-09-03 the mechanical half of that pass is `/audit-fleet`.** The hook could only
ever nag: it fires on a write, announces this file is stale, and has no way of knowing
whether anyone then checked anything — which is how three closed runs went unrecorded here
for three days. `graph_agents/.graph/audit-fleet.py` re-derives every checkable claim in
this file from disk and reports only what drifted. It does **not** stamp the date, for the
reason in the paragraph above; it earns you the mechanical half of the stamp paragraph and
nothing else. The prose — the gap list, the per-run narratives, the Decisions log — is
still read by a human or by an agent that says which parts it actually re-read.
...
19. **The headline contract is prose, nothing enforces it, and the board's delivery is
    ...
    **And the test corrected the design it was verifying.** The hook was written believing `Agent`
    ...
    real subagent stop between 3.5s and 1772s away — never once the other way round. Subagents run
    ...
    **dispatch** board: it shows the run as the node picks it up, before that node has written a
    ...
    `feature-graph` § the board previously said the opposite ("a node returning prints it for you,
...
20. **`activity.jsonl` is polluted with `SubagentStop` events that are not node stops.**
    ...
    second definition of "how long" grow next to `brief.py`'s.
21. **Twelve commits already in history carry Claude attribution, and the guard is
    prospective only.** Booked 2026-09-06, the day `guard-commit-trailers.py` closed the
    forward-looking half. Measured across all 11 sibling repos with `git log --all`:
    `graph_agents` **4** (all 2026-09-05, the review-attempts run), `koenrane.xyz` **7**
    (2026-07-21 to 2026-08-26), `whoop-med-tracker` **1** (its initial commit). Every other
    repo is clean, and **no repo has a commit authored by anyone but the owner** — the
    2026-08-26 re-authoring held; it is only trailers that came back. Cleaning these means
    `filter-repo`/`filter-branch` and a **force-push** on repos with remotes, which is the
    owner's call and not a thing to do unasked. Recorded here so the number is known rather
    than rediscovered: the guard stops the 13th, it cannot unmake the 12.
...
| 2026-09-03 | **The fleet auditor checks claims and never stamps the date** | `/audit-fleet` exists because "stale entries are worse than missing ones" was enforced by a hook that can only nag: it announces the obligation and cannot tell whether anyone discharged it. The obvious next step — have the script bump `Last verified:` once it goes green — is refused, because it would assert a verification that never happened for every prose claim in the file, which is the exact failure the header warns about. This does not overturn the 2026-08-25 do-not-revisit ruling; it is that ruling applied to a second tool |
| 2026-09-03 | **The auditor checks the unscoped half of a claim and reports the scoped half as evidence** | The roster's "Has executed?" column carries two claims in one cell. The yes/no is unscoped and load-bearing — gap #2 is entirely a statement about which nodes have never run — so it is machine-checked against written keys. The count beside it is explicitly scoped ("through the end of run 3"), so a later run outdates the number without making the claim false; checking it would report correct prose as drift, and a checker that cries wolf gets ignored, which is how the six dead hooks of gap #18 stayed dead |
| 2026-09-03 | **Node execution is counted from `state.json`, never from `activity.jsonl`** | The heartbeat looks like the natural source and is not: gap #20 says 471 of 503 recorded stop events are phantoms carrying an `agent_id` that never started. A written key is the fleet's own definition of a node having run, and `verify-state.py` owns what "written" means — imported rather than restated, so the auditor and the audit cannot drift into two definitions |
...
| 2026-09-03 | **The board is delivered from `PostToolUse`/`Agent`, never from `SubagentStop`** | `SubagentStop` is a *display event*: the docs put both its stdout and its `systemMessage` into Claude's context "instead of being shown in the transcript", so a board emitted there reaches the orchestrator — which already knows — and never the human. The fleet's own heartbeat gives the second reason: 471 of 503 stop events are phantoms (gap #20), so the event is not even a reliable "a node finished" signal here. `Agent` `PostToolUse` is non-display, so `systemMessage` surfaces — **verified by observation 2026-09-03**. *Corrected the same day: this entry originally said "exactly one per node return". It is one per node* ***dispatch*** — see the row below |
| 2026-09-03 | **Two boards per node, from two actors: the hook prints at dispatch, the orchestrator prints at return** | Measured, after the assumption behind the hook turned out to be wrong: all 32 `Agent` events in the heartbeat land within 0.2s of a `SubagentStart`, never near a node's own stop, because a spawn call returns a handle and the node runs in the background. Nothing can be done about that — the only event at a node's return is `SubagentStop`, which cannot address a human — so rather than lose the board that matters, the two are split by actor. The hook's dispatch board is free and shows what the node is picking up; the orchestrator's return board is manual and shows what it did. The rule that said "a node returning prints it for you, do not print it again" was written on the wrong premise and is inverted |
...
| 2026-09-11 | **A router is scoped around an open gap, not blocked on it — and never re-derives a metric a sibling tool already owns** | `/postmortem` was the one router of the 2026-09-03 brainstorm's three deliberately left unbuilt, because it reads `activity.jsonl` and gap #20 makes duration unsound. Building it anyway meant drawing the line explicitly: tool counts and tool-bounded (`start`/`tool`, never `stop`) windows are sound and go in; any `start`/`stop`-pair duration does not, full stop, until the gap closes. If it closes, duration is added to this script once — never computed a second, independent way that could quietly disagree with `brief.py`'s |
