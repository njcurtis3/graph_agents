...
| Plan-scope guard | live. **Fixed 2026-08-26 (`46e0f25`) to be worktree-aware** — it resolved plan entries against `repos/`, so it denied *every* write by *every* builder in a diamond. **Fixed again 2026-09-02: it did not glob.** A plan entry of `<dir>/**` normalised with its literal `**` attached, and since no real file is equal to or prefixed by a path ending in `**`, it denied *every* write under an approved directory — it blocked 3 of 4 slices of `2026-09-01-huntstack-mobile`, whose file sets were exactly that. Trailing `**`/`*` segments are now reduced to the directory they stand for at parse time, residual wildcards fall through to `fnmatch`, and both match sites share one `_match()` so they cannot drift apart a third time. **Now has a real self-test** (14 cases, previously the 14 "synthetic cases" lived only in a builder's transcript). **Exercised for real** across 9 builder runs with zero false denials since. ⚠️ **Matcher is `Write\|Edit` only, so a Bash write bypasses it entirely** — see gap #13. ⚠️ **Fails open**: `settings.json` wraps it in `\|\| true` and the script's own `main()` swallows every exception, so a crash is indistinguishable from "no opinion" and silently stops guarding — see gap #17 | `.claude/settings.json`, `.claude/hooks/guard-builder-scope.py` (279 ln), `.claude/hooks/test_guard_builder_scope.py` (194 ln) |
...
17. **The plan-scope guard fails open, and a crash is indistinguishable from "no opinion."**
    `settings.json:10` invokes it as `python ...guard-builder-scope.py 2>/dev/null || true`. For a
    `PreToolUse` hook exit 0 means *allow*, so `|| true` converts any crash into an allow and
    `2>/dev/null` discards the reason. The script's own `main()` does the same thing internally —
    every early return is an allow, and its docstring states "Exit 0 always" as intended design.
    That is defensible for the documented silences (no open run, run closed, plan lists no files),
    but it means **a syntax error in this file does not block builders, it silently stops guarding
    them**, and nothing anywhere reports it. The guard exists to enforce a human gate; a typo
    disabling that gate invisibly is the wrong failure direction. Discovered 2026-09-02 while
    fixing the glob bug — *not* fixed in the same change, because flipping a guard to fail-closed
    is a safety-posture decision for the owner, not a drive-by edit. Options: keep fail-open but
    drop `2>/dev/null` so errors surface; or have the hook catch its own exceptions and emit a
    deliberate `deny` naming itself as broken, which blocks only builders (the orchestrator and
    every other agent are already exempt) and is loud rather than silent. **Undecided.**
