...
18. **The other six hook commands are cwd-relative and fail silently when cwd is not `repos/`.**
    Found 2026-09-02 the moment the scope guard stopped swallowing its own errors: with the shell
    inside `graph_agents/`, `python graph_agents/.claude/hooks/...` resolves to
    `graph_agents/graph_agents/...` and the script is simply not found. The scope guard was fixed to
    `"${CLAUDE_PROJECT_DIR:-.}/graph_agents/..."` and **verified to work from a non-root cwd**
    (an `Edit` that failed from `graph_agents/` before the change succeeded after it, which is also
    what proves Claude Code sets `CLAUDE_PROJECT_DIR` for hooks). The staleness, cross-app-import,
    state-gap and three heartbeat entries were **left alone**: they all still carry `|| true`, so the
    same failure is silent there, and how long they have been quietly not running whenever cwd was
    not `repos/` is **unknown and unmeasured**. They are advisory `PostToolUse` hooks, so fail-open
    is defensible for them -- but a hook nobody can tell is dead is not much of a hook. Fixing them
    is a one-line-each change nobody has made yet.
