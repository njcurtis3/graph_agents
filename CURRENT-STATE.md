...
| Node heartbeat | live — `SubagentStart`/`SubagentStop` + a `*`-matcher `PostToolUse` entry append one line per node event to `.graph/runs/<run>/activity.jsonl`. Verified: events recorded with agent/tool, main-session writes recorded as `orchestrator`, silent on a closed run and with no open run. **Gained `parent` (`parent_tool_use_id`) 2026-09-09**, diagnostic for gap #20: 1,411 of 1,496 recorded `stop` events have no matching `start`, are stamped `orchestrator` for lack of `agent_type`, and interleave with live work — not what "a subagent finished" should look like. `parent` is logged, not yet read; the hypothesis it tests (a phantom nests under a real node's own `tool_use_id` rather than being orphaned) is unverified until a run produces one. **Gained `tokens` 2026-09-11** — a real per-agent-id running total, summed on every `tool`/`stop` event from that specific subagent's own transcript (`~/.claude/projects/<encoded cwd>/<session_id>/subagents/agent-<agent_id>.jsonl`, `message.usage` per turn), not from anything the hook payload itself carries — confirmed against the hooks reference that no usage field exists there at all. A real coupling to an undocumented, internal Claude Code storage layout, accepted deliberately (built for FleetView's per-node token counter) and unwritten on any read/parse failure rather than fabricating a count. 15 checks in the new `test_record_activity.py`: `encode_cwd` against the one observed real mapping, path construction, token summation against a synthetic transcript (real usage lines + a torn line + a no-usage line), and `main()` end-to-end with `HOME` and `CURRENT` redirected into a temp fleet — an orchestrator-side event (no `agent_id`) and a missing transcript both correctly omit `tokens` rather than raising or writing a fake 0. The transcript shape and location (`agent-<id>.jsonl` under a session's `subagents/`, `message.usage` per line) were confirmed against real files from past sessions on this machine before writing the reader; the hook itself has not yet fired against a live fleet run, since none was open while this was built | `.claude/settings.json`, `.claude/hooks/record-activity.py` (198 ln), `.claude/hooks/test_record_activity.py` (157 ln) |
