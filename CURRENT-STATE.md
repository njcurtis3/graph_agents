...
## Status: nine runs, seven of them executed; the first diamond has run
...
this file entirely until 2026-09-03**, when `/audit-fleet`'s first run found them; the
ninth is a throwaway probe, not a run.
...
| `2026-09-03-board-probe` | umbrella | n/a | **probe, not a run** — `status: building`, no work, exists only so a restarted session has an OPEN run for `show-board.py` to render (gap #19) | Delete it once the board has been observed rendering, and drop this row and the count above with it |

Run state lives at `graph_agents/.graph/runs/<run-id>/state.json` for each of the nine.
